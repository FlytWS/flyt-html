...
function setDisplayMode() {
	...
	if (localStorage.displayModeFlag == 1) {
		...
		document.getElementById('display-control-menu-item-mode').innerHTML = '<svg class="standard-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-moon"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>';
		document.getElementById('nav-mobile-burger-svg').style.color = "white";
		document.getElementById('nav-mobile-burder-page-title').style.color = "white";

	} else {
		displayModeFlag = 0;
		$('.map-tiles').css({"filter" : "none"});
		document.getElementById('display-control-menu-item-mode').innerHTML = '<svg class="standard-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-sun"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>';
		document.getElementById('nav-mobile-burger-svg').style.color = "black";
		document.getElementById('nav-mobile-burder-page-title').style.color = "black";
		...
		const newArray = acArray.map((currentValue, index, array)=>{
			...
			try {
				...
				if (currentValue.lat) {
					...
					icon: new L.DivIcon({
					className: 'infoPanel',
					html: '<img id="mapMarker-'+currentValue.hex+'" class="mapMarker" style="'+filter+' transform: rotate(' + currentValue.track + 'deg); width:'+ craftSizeArray[currentValue.category] +';" src="/flyt/resources/images/' + currentValue.category + '.svg"/>' 
					}),
			...
			if(resultFetchAR !== undefined) {
				...
				try {
					...
					document.getElementById('display-list-country-'+currentValue.hex).innerHTML = "<img title='" + resultFetchAR.response.aircraft.registered_owner_country_iso_name + "' src='/flyt/resources/images/flags/4x3/"+(resultFetchAR.response.aircraft.registered_owner_country_iso_name).toLowerCase()+".png' style='width:1rem;' />";
...
function drawOutlineJson() {
    ...
    request.done(function(data) {
			...
			polygon1.setStyle({fillColor: '#00000000', color: '#e484f6', weight: 2});
			
			polygon2 = L.polygon(RangeAltitude2).addTo(mapMap);
			polygon2.setStyle({fillColor: '#00000000', color: '#ff8582', weight: 2});
			
			polygon3 = L.polygon(RangeAltitude3).addTo(mapMap);
			polygon3.setStyle({fillColor: '#00000000', color: '#a9a934', weight: 2});
			
			polygon4 = L.polygon(RangeAltitude4).addTo(mapMap);
			polygon4.setStyle({fillColor: '#00000000', color: '#42c26b', weight: 2});
			
			polygon9 = L.polygon(RangeAltitude9).addTo(mapMap);
			polygon9.setStyle({fillColor: '#00000000', color: '#FFFFFF', weight: 2});
